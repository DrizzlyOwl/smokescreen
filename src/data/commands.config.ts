import type { Command, CommandActions } from '../hooks/useCommandRegistry';
import { useIncidentStore } from '../store/useIncidentStore';
import { NodeType } from '../utils/nodeTypes';

export const getCommands = (actions: CommandActions): Command[] => [
    // Panes
    {
      id: 'chat_on',
      patterns: ['set chat on', 'chat', 'warroom'],
      action: () => actions.openPane('chat'),
      description: 'Display incident response chat connection',
      category: 'PANES',
      usage: 'set chat on',
      confirmation: 'OPENING_CHAT_CONNECTION... [OK]',
    },
    {
      id: 'chat_off',
      patterns: ['set chat off'],
      action: () => actions.closePane('chat'),
      description: 'Close incident response chat connection',
      category: 'PANES',
      usage: 'set chat off',
      confirmation: 'TERMINATING_CHAT_CONNECTION... [OK]',
    },
    {
      id: 'terminal_on',
      patterns: ['set terminal on', 'terminal', 'console', 'shell'],
      action: () => actions.openPane('terminal'),
      description: 'Display secondary system terminal',
      category: 'PANES',
      usage: 'set terminal on',
      confirmation: 'INITIALIZING_SECONDARY_SHELL... [OK]',
    },
    {
      id: 'terminal_off',
      patterns: ['set terminal off'],
      action: () => actions.closePane('terminal'),
      description: 'Close secondary system terminal',
      category: 'PANES',
      usage: 'set terminal off',
      confirmation: 'TERMINATING_SECONDARY_SHELL... [OK]',
    },
    {
      id: 'logs_on',
      patterns: ['set logs on', 'logs', 'kernel'],
      action: () => actions.openPane('logs'),
      description: 'Display kernel log stream',
      category: 'PANES',
      usage: 'set logs on',
      confirmation: 'STREAMING_KERNEL_BUFFER... [OK]',
    },
    {
      id: 'logs_off',
      patterns: ['set logs off'],
      action: () => actions.closePane('logs'),
      description: 'Close kernel log stream',
      category: 'PANES',
      usage: 'set logs off',
      confirmation: 'HALTING_KERNEL_BUFFER_STREAM... [OK]',
    },
    {
      id: 'map_on',
      patterns: ['set map on', 'map', 'outage'],
      action: () => actions.openPane('map'),
      description: 'Display global outage map',
      category: 'PANES',
      usage: 'set map on',
      confirmation: 'MAPPING_GLOBAL_OUTAGE_DATA... [OK]',
    },
    {
      id: 'map_off',
      patterns: ['set map off'],
      action: () => actions.closePane('map'),
      description: 'Close global outage map',
      category: 'PANES',
      usage: 'set map off',
      confirmation: 'DISCONNECTING_GEOSPATIAL_FEED... [OK]',
    },
    {
      id: 'deploy_on',
      patterns: ['set deploy on', 'set k8s on', 'deploy', 'k8s'],
      action: () => actions.openPane('deploy'),
      description: 'Display K8s deployment status',
      category: 'PANES',
      usage: 'set deploy on',
      confirmation: 'POLLING_K8S_DEPLOYMENT_STATUS... [OK]',
    },
    {
      id: 'deploy_off',
      patterns: ['set deploy off', 'set k8s off'],
      action: () => actions.closePane('deploy'),
      description: 'Close K8s deployment status',
      category: 'PANES',
      usage: 'set deploy off',
      confirmation: 'TERMINATING_K8S_POLLING_THREAD... [OK]',
    },
    {
      id: 'burn_on',
      patterns: ['set burn on', 'burn', 'money', 'cost'],
      action: () => actions.openPane('burn'),
      description: 'Display real-time infrastructure burn rate dashboard',
      category: 'PANES',
      usage: 'set burn on',
      confirmation: 'CALCULATING_INFRASTRUCTURE_BURN_RATE... [OK]',
    },
    {
      id: 'burn_off',
      patterns: ['set burn off'],
      action: () => actions.closePane('burn'),
      description: 'Close burn rate dashboard',
      category: 'PANES',
      usage: 'set burn off',
      confirmation: 'SUSPENDING_COST_ANALYSIS_ENGINE... [OK]',
    },
    {
      id: 'howto_on',
      patterns: ['set howto on', 'how to', 'howto', 'manual'],
      action: () => actions.openPane('howTo'),
      description: 'Display official Smokescreen game manual',
      category: 'PANES',
      usage: 'set howto on',
      confirmation: 'OPENING_SMOKESCREEN_GAME_MANUAL... [OK]',
    },
    {
      id: 'howto_off',
      patterns: ['set howto off'],
      action: () => actions.closePane('howTo'),
      description: 'Close game manual',
      category: 'PANES',
      usage: 'set howto off',
      confirmation: 'CLOSING_GAME_MANUAL... [OK]',
    },
    {
      id: 'settings_on',
      patterns: ['set settings on', 'settings', 'config'],
      action: () => actions.openPane('settings'),
      description: 'Display system configuration',
      category: 'PANES',
      usage: 'set settings on',
      confirmation: 'ACCESSING_SYSTEM_CONFIGURATION... [OK]',
    },
    {
      id: 'settings_off',
      patterns: ['set settings off'],
      action: () => actions.closePane('settings'),
      description: 'Close system configuration',
      category: 'PANES',
      usage: 'set settings off',
      confirmation: 'EXITING_SYSTEM_CONFIGURATION... [OK]',
    },
    {
      id: 'debug_on',
      patterns: ['set debug on', 'debug'],
      action: () => actions.openPane('debug'),
      description: 'Display system debug console',
      category: 'PANES',
      usage: 'set debug on',
      confirmation: 'ACCESSING_SYSTEM_DEBUG_HOOKS... [OK]',
    },
    {
      id: 'debug_off',
      patterns: ['set debug off'],
      action: () => actions.closePane('debug'),
      description: 'Close system debug console',
      category: 'PANES',
      usage: 'set debug off',
      confirmation: 'DISENGAGING_DEBUG_HOOKS... [OK]',
    },
    {
      id: 'all_on',
      patterns: ['set all on', 'all'],
      action: () => actions.openAll(),
      description: 'Restore all observability panes',
      category: 'PANES',
      usage: 'set all on',
      confirmation: 'RESTORING_ALL_OBSERVABILITY_PANES... [OK]',
    },
    {
      id: 'all_off',
      patterns: ['set all off', 'clear'],
      action: () => actions.closeAll(),
      description: 'Clear workspace viewport',
      category: 'PANES',
      usage: 'set all off',
      confirmation: 'CLEARING_WORKSPACE_VIEWPORT... [OK]',
    },

    // Threat
    {
      id: 'nominal',
      patterns: ['set threat nominal', 'nominal', 'reset'],
      action: () => actions.setSeverity('NOMINAL'),
      description: 'Reset systems to nominal state',
      category: 'THREAT',
      usage: 'set threat nominal',
      confirmation: 'ALL_SYSTEMS_NOMINAL... [OK]',
    },
    {
      id: 'p3',
      patterns: ['set threat p3', 'p3'],
      action: () => actions.setSeverity('P3'),
      description: 'Escalate to P3 (Degraded)',
      category: 'THREAT',
      usage: 'set threat p3',
      confirmation: 'INCIDENT_ESCALATED: P3 [DEGRADED_STATE]',
    },
    {
      id: 'p1',
      patterns: ['set threat p1', 'p1'],
      action: () => actions.setSeverity('P1'),
      description: 'Escalate to P1 (Critical)',
      category: 'THREAT',
      usage: 'set threat p1',
      confirmation: 'INCIDENT_ESCALATED: P1 [CRITICAL_ALERT]',
    },
    {
      id: 'p0',
      patterns: ['set threat p0', 'p0'],
      action: () => actions.setSeverity('P0'),
      description: 'Escalate to P0 (Catastrophic)',
      category: 'THREAT',
      usage: 'set threat p0',
      confirmation: 'INCIDENT_ESCALATED: P0 [TOTAL_SYSTEM_OUTAGE]',
    },

    // Stack
    {
      id: 'aws',
      patterns: ['set stack aws', 'aws'],
      action: () => actions.setStack('AWS'),
      description: 'Set stack to Amazon Web Services',
      category: 'STACK',
      usage: 'set stack aws',
      confirmation: 'PRIMARY_STACK_SET: AWS [SUCCESS]',
    },
    {
      id: 'gcp',
      patterns: ['set stack gcp', 'gcp'],
      action: () => actions.setStack('GCP'),
      description: 'Set stack to Google Cloud Platform',
      category: 'STACK',
      usage: 'set stack gcp',
      confirmation: 'PRIMARY_STACK_SET: GCP [SUCCESS]',
    },
    {
      id: 'azure',
      patterns: ['set stack azure', 'azure'],
      action: () => actions.setStack('AZURE'),
      description: 'Set stack to Microsoft Azure',
      category: 'STACK',
      usage: 'set stack azure',
      confirmation: 'PRIMARY_STACK_SET: AZURE [SUCCESS]',
    },
    {
      id: 'onprem',
      patterns: ['set stack onprem', 'onprem'],
      action: () => actions.setStack('ON-PREM'),
      description: 'Set stack to On-Premise Hardware',
      category: 'STACK',
      usage: 'set stack onprem',
      confirmation: 'PRIMARY_STACK_SET: ON-PREM [SUCCESS]',
    },
    {
      id: 'serverless',
      patterns: ['set stack serverless', 'serverless'],
      action: () => actions.setStack('SERVERLESS'),
      description: 'Set stack to Serverless Runtime',
      category: 'STACK',
      usage: 'set stack serverless',
      confirmation: 'PRIMARY_STACK_SET: SERVERLESS [SUCCESS]',
    },
    {
      id: 'cloudflare',
      patterns: ['set stack cloudflare', 'set stack cf', 'cloudflare', 'cf'],
      action: () => actions.setStack('CLOUDFLARE'),
      description: 'Set stack to Cloudflare Edge',
      category: 'STACK',
      usage: 'set stack cloudflare',
      confirmation: 'PRIMARY_STACK_SET: CLOUDFLARE [SUCCESS]',
    },
    {
      id: 'heroku',
      patterns: ['set stack heroku', 'heroku'],
      action: () => actions.setStack('HEROKU'),
      description: 'Set stack to Heroku PaaS',
      category: 'STACK',
      usage: 'set stack heroku',
      confirmation: 'PRIMARY_STACK_SET: HEROKU [SUCCESS]',
    },
    {
      id: 'hyperv',
      patterns: ['set stack hyperv', 'set stack hyper-v', 'hyperv', 'hyper-v'],
      action: () => actions.setStack('HYPER-V'),
      description: 'Set stack to Microsoft Hyper-V',
      category: 'STACK',
      usage: 'set stack hyperv',
      confirmation: 'PRIMARY_STACK_SET: HYPER-V [SUCCESS]',
    },
    {
      id: 'vmware',
      patterns: ['set stack vmware', 'set stack vsphere', 'vmware', 'vsphere'],
      action: () => actions.setStack('VMWARE'),
      description: 'Set stack to VMWare vSphere',
      category: 'STACK',
      usage: 'set stack vmware',
      confirmation: 'PRIMARY_STACK_SET: VMWARE [SUCCESS]',
    },

    // System
    {
      id: 'audio_on',
      patterns: ['set audio on', 'audio on'],
      action: () => actions.setAudio(true),
      description: 'Enable system audio engine',
      category: 'SYSTEM',
      usage: 'set audio on',
      confirmation: 'SYSTEM_AUDIO_RE-ENABLED... [OK]',
    },
    {
      id: 'audio_off',
      patterns: ['set audio off', 'audio off'],
      action: () => actions.setAudio(false),
      description: 'Disable system audio engine',
      category: 'SYSTEM',
      usage: 'set audio off',
      confirmation: 'SYSTEM_AUDIO_SILENCED... [OK]',
    },
    {
      id: 'slowburn_on',
      patterns: ['set slowburn on', 'slowburn on', 'slowburn'],
      action: () => actions.setSlowBurn(true),
      description: 'Engage slow burn protocol',
      category: 'SYSTEM',
      usage: 'set slowburn on',
      confirmation: 'SLOW_BURN_PROTOCOL_ENGAGED... [ARMED]',
    },
    {
      id: 'slowburn_off',
      patterns: ['set slowburn off', 'slowburn off'],
      action: () => actions.setSlowBurn(false),
      description: 'Disengage slow burn protocol',
      category: 'SYSTEM',
      usage: 'set slowburn off',
      confirmation: 'SLOW_BURN_PROTOCOL_DISENGAGED... [DISARMED]',
    },
    {
      id: 'theme_classic',
      patterns: ['set theme classic', 'theme classic', 'theme green', 'classic'],
      action: () => actions.setTheme('classic'),
      description: 'Set theme to Classic Green',
      category: 'SYSTEM',
      usage: 'set theme classic',
      confirmation: 'VISUAL_THEME_SET: CLASSIC_GREEN [OK]',
    },
    {
      id: 'theme_amber',
      patterns: ['set theme amber', 'theme amber', 'amber'],
      action: () => actions.setTheme('amber'),
      description: 'Set theme to CRT Amber',
      category: 'SYSTEM',
      usage: 'set theme amber',
      confirmation: 'VISUAL_THEME_SET: CRT_AMBER [OK]',
    },
    {
      id: 'theme_cobalt',
      patterns: ['set theme cobalt', 'theme cobalt', 'theme blue', 'cobalt', 'blue'],
      action: () => actions.setTheme('cobalt'),
      description: 'Set theme to Deep Cobalt',
      category: 'SYSTEM',
      usage: 'set theme cobalt',
      confirmation: 'VISUAL_THEME_SET: DEEP_COBALT [OK]',
    },
    {
      id: 'theme_dracula',
      patterns: ['set theme dracula', 'theme dracula', 'theme purple', 'theme neovim', 'dracula', 'purple'],
      action: () => actions.setTheme('dracula'),
      description: 'Set theme to Dracula Neovim',
      category: 'SYSTEM',
      usage: 'set theme dracula',
      confirmation: 'VISUAL_THEME_SET: DRACULA_NEOVIM [OK]',
    },
    {
      id: 'theme_monokai',
      patterns: ['set theme monokai', 'theme monokai', 'theme vsc', 'monokai', 'vsc'],
      action: () => actions.setTheme('monokai'),
      description: 'Set theme to Monokai VSC',
      category: 'SYSTEM',
      usage: 'set theme monokai',
      confirmation: 'VISUAL_THEME_SET: MONOKAI_VSC [OK]',
    },
    {
      id: 'theme_cyberpunk',
      patterns: ['set theme cyberpunk', 'theme cyberpunk', 'cyberpunk', 'neon'],
      action: () => actions.setTheme('cyberpunk'),
      description: 'Set theme to Cyberpunk Neon',
      category: 'SYSTEM',
      usage: 'set theme cyberpunk',
      confirmation: 'VISUAL_THEME_SET: CYBERPUNK_NEON [OK]',
    },
    {
      id: 'theme_high_contrast',
      patterns: ['set theme contrast', 'theme contrast', 'contrast', 'mono'],
      action: () => actions.setTheme('high-contrast'),
      description: 'Set theme to High-Contrast (Monochromatic)',
      category: 'SYSTEM',
      usage: 'set theme contrast',
      confirmation: 'VISUAL_THEME_SET: HIGH_CONTRAST [OK]',
    },
    {
      id: 'eco_on',
      patterns: ['set eco on', 'eco on', 'eco engage'],
      action: () => actions.setEcoMode(true),
      description: 'Enable system eco mode',
      category: 'SYSTEM',
      usage: 'set eco on',
      confirmation: 'ECO_MODE_ENGAGED... [OK]',
    },
    {
      id: 'eco_off',
      patterns: ['set eco off', 'eco off', 'eco disengage'],
      action: () => actions.setEcoMode(false),
      description: 'Disable system eco mode',
      category: 'SYSTEM',
      usage: 'set eco off',
      confirmation: 'ECO_MODE_DISENGAGED... [OK]',
    },


    // Action
    {
      id: 'analyze',
      patterns: ['analyze', 'diagnose', 'sitrep'],
      action: () => {
        actions.generateStrategy();
        actions.openPane('readout');
        actions.clearInterruption();
      },
      description: 'Run automated diagnostics and generate response strategy',
      category: 'ACTION',
      usage: 'analyze',
      confirmation: 'INITIATING_SYSTEM_DIAGNOSTICS... GENERATING_STRATEGY... [OK]',
    },
    {
      id: 'declare',
      patterns: ['declare', 'emergency', 'panic'],
      action: () => actions.handleEject(),
      description: 'Declare critical incident theatre',
      category: 'ACTION',
      usage: 'declare',
      confirmation: 'CRITICAL_INCIDENT_DECLARED... [EMERGENCY_OVERRIDE_ACTIVE]',
    },
    {
      id: 'k8s_restart',
      patterns: ['k8s restart', 'kubectl restart'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.K8S);
        useIncidentStore.getState().setIsDeployStabilized(true);
        useIncidentStore.getState().incrementMitigationCount();
      },
      description: 'Stabilize K8s cluster workloads',
      category: 'ACTION',
      usage: 'k8s restart',
      confirmation: 'RESTARTING_K8S_WORKLOADS... [OK]',
    },
    {
      id: 'cease',
      patterns: ['cease', 'resolve', 'restore', 'abort'],
      action: () => actions.handleCease(),
      description: 'Resolve incident and restore nominal state',
      category: 'ACTION',
      usage: 'resolve',
      confirmation: 'ALL_SYSTEMS_RESTORED... [NOMINAL_STATE]',
    },
    {
      id: 'copy',
      patterns: ['copy', 'clipboard'],
      action: () => actions.copyPlaybook(),
      description: 'Copy active incident playbook to clipboard',
      category: 'ACTION',
      usage: 'copy',
      confirmation: 'COPYING_PLAYBOOK_TO_CLIPBOARD... [OK]',
    },
    {
      id: 'ticket',
      patterns: ['ticket', 'restricted'],
      action: () => actions.setView('TICKET'),
      description: 'View restricted incident ticket',
      category: 'ACTION',
      usage: 'ticket',
      confirmation: 'ACCESSING_RESTRICTED_TICKET_VIEW... [OK]',
    },
    {
      id: 'help',
      patterns: ['help', '/?', '?', 'man'],
      action: () => actions.help(getCommands(actions)),
      description: 'Display system command manifest',
      category: 'ACTION',
      usage: 'help [category]',
      confirmation: 'LOADING_COMMAND_MANIFEST... [OK]',
    },
    {
      id: 'scenario',
      patterns: ['scenario'],
      action: (ctx) => actions.startScenario(ctx.arg as string),
      description: 'Trigger automated scenario simulation',
      category: 'ACTION',
      usage: 'scenario <id>',
      confirmation: 'INITIALIZING_SCENARIO: $arg... [OK]',
    },
    {
      id: 'override_test',
      patterns: ['override_test', 'trigger_puzzle'],
      action: (ctx) => {
        const type = ctx.arg as 'phrase' | 'hold' | undefined;
        actions.triggerApproval(type);
      },
      description: 'Force-trigger a critical action puzzle',
      category: 'ACTION',
      usage: 'override_test [phrase|hold]',
      confirmation: 'TRIGGERING_MANUAL_OVERRIDE_WORKFLOW... [OK]',
    },
    {
      id: 'logout',
      patterns: ['logout', 'shutdown', 'exit', 'quit'],
      action: () => actions.handleLogout(),
      description: 'Terminate session and logout',
      category: 'SYSTEM',
      usage: 'logout',
      confirmation: 'TERMINATING_SESSION... BYE.',
    },
    {
      id: 'patch_confirm',
      patterns: ['patch confirm', 'patch-confirm'],
      action: () => actions.incrementMitigationCount(),
      description: 'Confirm successful AMI patch rotation',
      category: 'ACTION',
      usage: 'patch confirm',
      confirmation: 'PATCH_ROTATION_CONFIRMED... [OK]',
    },
    {
      id: 'bgp_reset',
      patterns: ['bgp reset', 'bgp-reset'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.BGP);
        actions.incrementMitigationCount();
      },
      description: 'Emergency reset of BGP transit sessions',
      category: 'ACTION',
      usage: 'bgp reset',
      confirmation: 'BGP_SESSION_RESET_COMPLETE... [OK]',
    },
    {
      id: 'db_kill',
      patterns: ['db kill', 'db-kill'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.DB);
        actions.incrementMitigationCount();
      },
      description: 'Force-kill locked database queries',
      category: 'ACTION',
      usage: 'db kill',
      confirmation: 'LOCKED_QUERIES_TERMINATED... [OK]',
    },
    {
      id: 'mesh_restart',
      patterns: ['mesh restart', 'mesh-restart'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.MESH);
        actions.incrementMitigationCount();
      },
      description: 'Soft-restart service mesh control plane',
      category: 'ACTION',
      usage: 'mesh restart',
      confirmation: 'SERVICE_MESH_RESTARTING... [OK]',
    },
    {
      id: 'vault_seal',
      patterns: ['vault seal', 'vault-seal'],
      action: (context) => {
        const token = actions.getDiagnosticToken?.();
        if (token && context.arg?.toUpperCase() !== token.toUpperCase()) {
            return { isValid: false, message: 'ERROR: AUTH REQUIRED. SEE SYSTEM LOGS [F2] FOR DIAGNOSTIC_TOKEN.' };
        }
        useIncidentStore.getState().healNodes(NodeType.VAULT);
        actions.incrementMitigationCount();
      },
      description: 'Emergency seal of kernel secret vault',
      category: 'ACTION',
      usage: 'vault seal <token>',
      confirmation: 'SECRET_VAULT_SEALED_IMMEDIATELY... [OK]',
    },
    {
      id: 'probe_override',
      patterns: ['probe override', 'probe-override'],
      action: (context) => {
        const token = actions.getDiagnosticToken?.();
        if (token && context.arg?.toUpperCase() !== token.toUpperCase()) {
            return { isValid: false, message: 'ERROR: AUTH REQUIRED. SEE SYSTEM LOGS [F2] FOR DIAGNOSTIC_TOKEN.' };
        }
        useIncidentStore.getState().healNodes(NodeType.PROBE);
        actions.incrementMitigationCount();
      },
      description: 'Manual override for service topology probes',
      category: 'ACTION',
      usage: 'probe override <token>',
      confirmation: 'PROBE_OVERRIDE_ACCEPTED... [OK]',
    },
    {
      id: 'compute_cycle',
      patterns: ['compute cycle', 'compute-cycle'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.COMPUTE);
        actions.incrementMitigationCount();
      },
      description: 'Emergency power cycle for compute nodes',
      category: 'ACTION',
      usage: 'compute cycle',
      confirmation: 'COMPUTE_NODES_CYCLING... [OK]',
    },
    {
      id: 'cdn_purge',
      patterns: ['cdn purge', 'cdn-purge'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.CDN);
        actions.incrementMitigationCount();
      },
      description: 'Global cache purge for edge network',
      category: 'ACTION',
      usage: 'cdn purge',
      confirmation: 'CDN_CACHE_PURGE_INITIATED... [OK]',
    },
    {
      id: 'queue_flush',
      patterns: ['queue flush', 'queue-flush'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.QUEUE);
        actions.incrementMitigationCount();
      },
      description: 'Emergency flush of message queue buffers',
      category: 'ACTION',
      usage: 'queue flush',
      confirmation: 'MESSAGE_QUEUES_FLUSHED... [OK]',
    },
    {
      id: 'network_reset',
      patterns: ['network reset', 'network-reset'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.NETWORK);
        actions.incrementMitigationCount();
      },
      description: 'Soft reset of virtual network interfaces',
      category: 'ACTION',
      usage: 'network reset',
      confirmation: 'VIRTUAL_NETWORK_RESET_COMPLETE... [OK]',
    },
    {
      id: 'storage_sync',
      patterns: ['storage sync', 'storage-sync'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.STORAGE);
        actions.incrementMitigationCount();
      },
      description: 'Force state synchronization for block storage',
      category: 'ACTION',
      usage: 'storage sync',
      confirmation: 'STORAGE_SYNC_COMPLETE... [OK]',
    },
    {
      id: 'system_override',
      patterns: ['system override', 'system-override'],
      action: () => {
        useIncidentStore.getState().healNodes(NodeType.UNKNOWN);
        actions.incrementMitigationCount();
      },
      description: 'Manual override for unknown system nodes',
      category: 'ACTION',
      usage: 'system override',
      confirmation: 'SYSTEM_OVERRIDE_ACCEPTED... [OK]',
    },
];
